...
resource docintelopenaiuserassignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  scope: account // Use when specifying a scope that is different than the deployment scope
  name: guid(subscription().id, resourceGroup().id, principalId, openAiUserRole, principalType, 'docintel')
  properties: {
